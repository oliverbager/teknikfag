\section{Teori}
    ...
    \begin{figure}[H]\centering
        ...
        \schemestop
        \chemmove{
            \node at (-12.5,-4.75) {\textit{ortho}--};
            \node at (-6.75,-4.75) {\textit{meta}--};
            \node at (-1,-4.75) {\textit{para}--};
        } \vskip 24pt
    ...
    Dette gøres ved at den stærke syre overfører en proton til det carbonyle oxygen--atom hvilket er muligt grundet elektronegativitetsforskellen. Dette medfører en stigning i det carbonyle carbon--atoms elektrofilicitet ved dannelse af en carbokat--ion. \vskip 8pt
    \begin{figure}[H]
        \resizebox{\linewidth}{!}{
            \schemestart
    ...
    \end{figure}
    Da energidifferensen mellem de to niveauer er relativt lille, er populationsforskellen det også, da simple kollisioner mellem molekyler ofte vil være nok til at skubbe et molekyle op i et højere energistadie. Teoretisk kan populationsforksellen beskrives som en Boltzmann-fordeling givet ved:
    \[
    ...
    Dette medfører at vores bulk magnetiseringsvektor ``tippes'' ud af ækvilibrium, når $n_{\text{høj}}=n_{\text{lav}}$ vil $\overrightarrow{M_z}=0$, hvorved vi har en nulvektor. Dog skubber $B_1$ samtidigt alle de magnetiske momenter i samme fase, hvilket medfører at vi har en magnitude i $\overrightarrow{M_{xy}}$--retningen. Hvorefter den, når påvirkningen fra $B_1$ ophører, vil begynde at præcessere omkring $z$--aksen mens $\overrightarrow{M_{xy}}\rightarrow 0$ og $\overrightarrow{M_z}\rightarrow M_{\text{start}}$ \parencite{Farr2007}. Vinklen magnetiseringsvektorne forskydes med ift.\ $z$--aksen er givet ved:
    \[
    ...
    \end{figure}
    For denne præcession defineres to tidsintervaller der beskriver kritiske punkter af den spirallignendebevægelse
    \begin{itemize}
    ...
    \end{figure}
    Grafen der dannes med udgangspunkt i den inducerede spænding kaldes \textit{FID}  (free induction decay), og giver os et eksponentielt faldende oscillerende signal, der ved Fourier transformation kan flyttes fra tidsdomænet, til frekvensdomænet \parencite{Davi2022}.

    I praksis gentages denne måling en stor mængde gange for at minimere mængden af støj i det endelige spektra. Dette gøres ved at udsætte prøven for $B_1$, og derefter vente i perioden svarende til $t_1$, hvorefter processen gentages indtil dataene er tilstrækkeligt klare.
    \begin{figure}[H]
    ...
    Dog er dette ikke altid tilfældet da der er mulighed for kompleks kobling mellem protoner på atomer, der ikke er lige ved siden af hinanden. Dette er specielt relevant når det kommer til aromatiske ringe.
    ...
    Under IR--spektroskopi bestråles en prøve med infrarøde bølger. Hvis de præcis matcher enegidifferensen mellem molekylets grundstadie og exciterede stadie, sker der et spring i energistadie:
    \[
    ...
    I takt med at flere atomer tilføjes kan vibrationerne dog kombineres på forskellige måder, samt påvirke hinanden, hvilket medfører en uendelig mængde mulige vibrationer.
    \begin{figure}[H]
        \resizebox{\linewidth}{!}{
            \begin{tikzpicture}
    ...
    \end{figure}
    Bøjninger kan ske både i eget plan og vinkelret derpå hvis molekylet er lineært (eksempelvis CO$_2$). Dvs.\ at nogle molekyler ikke udelukkende har mulighed for at bøje fra side til side, men også frem og tilbage. Disse vibrationer har dog ikke forskellige associerede energiniveauer, og de kan derfor være svære at identificere. Derudover bevæger det centrale molekyle sig også, da molekylets massemidtpunkt \textit{ikke} kan forskydes af vibrationerne.

    Mængden af mulige vibrationer er afhængigt af hvor mange frihedsgrader molekylet har. Idet atomer bevæger sig i et 3--dimensionelt rum, må hvert atom have 3 frihedsgrader, altså vil et molekyle have $3n$ frihedsgrader, hvor $n$ er mængden af atomer i molekylet.
    ...
    \end{figure}
    For et ikke-lineært triatomigt molekyle, eksempelvis H$_2$O, vil vi i stedet hve $3\cdot 3-6=3$ frihedsgrader, der korresponderer til hhv.\ den symmetriske-- og asymmetriske strækning af de kovalente bånd, samt bøjning af bindingen til hydrogen--atomerne \parencite{Theo2012}.
    \begin{figure}[H]
        \resizebox{\linewidth}{!}{
        \begin{tikzpicture}    
    ...
    \begin{table}[H]\centering
        ...
        \begin{tabular}{ccccc}
            \toprule
            & \chemfig{N-[0,.6]H} / \chemfig{O-[0,.6]H} & \chemfig{C-[0,.6]H} & \chemfig{C~[0,.6]N} / \chemfig{C~[0,.6]C} & \chemfig{C=[0,.6]O} \\
            \midrule
            Zone $\left[\nu\right]$ & 3200--4000 & 2800--3310 & 2000--2500 & 1550--1870 \\
            \bottomrule
        \end{tabular}
        }
    \end{table}
